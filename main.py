...
def modelo():
    ...
    model.addConstr(quicksum((x[l, t] * gen1[l]) for l in L for t in range(T)) >= req)

    #2. Todos lo proyectos deben terminarse a mas tardar en diciembre de 2049
    model.addConstrs(x[l, t] + plazo_g[l] <= 50 for l in L for t in range(T))
    ...
    model.addConstrs(quicksum(y[l, t] * emp_g[l, e] for l in L) + quicksum(z[n, t] * emp_n[n, e] for n in N) <= cap for e in E for t in range(T))
